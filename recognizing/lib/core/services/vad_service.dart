...
class VadService {
  ...
  static Future<void> initialize() async {
    ...
    try {
      _vad = await Vad.create(
        modelPath: await VadModelLoader.load(),
        sampleRate: _sampleRate,
        frameSize: _frameSize,
        mode: VadMode.AGGRESSIVE, // Good for voice typing
      );
  ...
  static Future<void> startListening({
    ...
    // Note: In a real implementation, you'd need to handle real-time audio stream
  ...
  static double? processAudioChunk(List<double> audioData) {
    if (!_isInitialized || _vad == null || !_isListening) {
      return null;
    }

    try {
      // This is a placeholder - actual VAD processing would happen here
      // The vad package provides methods for this
      return _calculateSpeechProbability(audioData);
    } catch (e) {
      if (kDebugMode) {
  ...
  static double _calculateSpeechProbability(List<double> audioData) {
    ...
    // In production, you'd use the actual VAD model
    ...
    final rms = sum > 0 ? (sum / audioData.length).sqrt() : 0;
  ...
  /// Optimize recording parameters based on VAD
  static RecordConfig optimizeRecordingConfig(RecordConfig baseConfig) {
    return RecordConfig(
      encoder: AudioEncoder.pcm16bit, // Use PCM for better VAD processing
      sampleRate: _sampleRate,
