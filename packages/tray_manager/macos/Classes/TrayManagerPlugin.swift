...
public class TrayManagerPlugin: NSObject, FlutterPlugin, NSMenuDelegate {
    ...
    public func popUpContextMenu(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        if let menu = trayMenu, let button = trayIcon?.statusItem?.button {
            // Set the menu temporarily (menuDidClose will clear it)
            trayIcon?.statusItem?.menu = menu
            // Use NSMenu.popUp directly instead of performClick — performClick
            // fails on macOS 16+ when called from a MethodChannel callback
            // because the original mouse event has already been consumed.
