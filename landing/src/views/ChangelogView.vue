...
const filteredVersions = computed(() => {
  return changelog.value.versions.filter((version: Version) => {
    ...
    return (
      ...
      version.highlights.some((h: string) => h.toLowerCase().includes(query)) ||
...
<template>
  <div class="min-h-screen pt-28 sm:pt-36 pb-16 lg:pb-24">
    <div class="container-custom max-w-4xl">
      <!-- Header -->
      <div class="text-center mb-12">
        <h1 class="text-4xl sm:text-5xl font-bold mb-4 text-slate-950 dark:text-slate-50">
          What's New
        </h1>
      ...
      <div class="mb-10 space-y-4">
        <div class="relative max-w-md mx-auto">
          <Search :size="16" class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
          <input
            v-model="searchQuery"
            type="text"
        ...
        <div class="flex flex-wrap justify-center gap-1.5">
          <button
            ...
            :class="[
              selectedCategory === null
                ...
                : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:border-slate-700'
            ]"
          >
            All
          </button>
          <button
            ...
            :key="categoryId"
            @click="selectedCategory = selectedCategory === categoryId ? null : (categoryId as string)"
          ...
          >
            {{ cat.label }}
            <span
              v-if="categoryCounts[categoryId as string]"
              class="px-1.5 py-0.5 rounded text-[10px]"
              :class="selectedCategory === categoryId ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800'"
            >
              {{ categoryCounts[categoryId as string] }}
            </span>
          </button>
        </div>
      </div>
      ...
      <div class="space-y-8">
        ...
        >
          <!-- Timeline -->
          <div class="absolute left-0 top-0 bottom-0 w-px bg-slate-200 dark:bg-slate-800" />
          <div class="absolute left-[-3px] top-2 w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-slate-600" />
          ...
          <div class="flex flex-wrap items-center gap-3 mb-4">
            ...
            <span
              v-if="version.stable"
              class="px-2 py-0.5 rounded text-[10px] font-medium bg-emerald-50 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400"
            >
              Stable
            </span>
          ...
          <!-- Highlights -->
          <div v-if="version.highlights?.length" class="mb-4 pl-4">
            ...
            >
              <div>
                <h3
                  ...
                  :class="getCategoryInfo(category as string).color + ' text-white'"
                >
                ...
                <ul class="space-y-1 pl-4">
                  ...
                  >
                    ...
                    <span v-if="change.description"> &mdash; {{ change.description }}</span>
                  </li>
                </ul>
              </div>
            </template>
      ...
      <div v-if="filteredVersionsByCategory.length === 0" class="text-center py-16">
        <Frown :size="40" class="mx-auto mb-4 text-slate-300 dark:text-slate-700" />
      ...
      <div class="mt-12 text-center">
        <a
          href="https://github.com/xicv/recogniz.ing/blob/main/CHANGELOG.md"
          target="_blank"
          rel="noopener"
          class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors"
        >
